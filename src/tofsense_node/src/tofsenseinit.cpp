...
NTS_ProtocolFrame0::NTS_ProtocolFrame0()
    : NLinkProtocol(true, g_nts_frame0.fixed_part_size,
                    {g_nts_frame0.frame_header,
                     g_nts_frame0.function_mark}) {}
...
Init::Init(
  ...
  is_inquire_mode_ =
      serial_
          ? node_->declare_parameter<bool>("inquire_mode", true)
          : false;
  ...
  protocol_frame0_->SetHandleDataCallback([this, protocol_frame0_]() {
    if (!publishers_frame0_[protocol_frame0_]) {
      if (is_inquire_mode_) {
        ...
        publishers_frame0_[protocol_frame0_] =
            node_->create_publisher<
                nlink_parser::msg::TofsenseFrame0>(
