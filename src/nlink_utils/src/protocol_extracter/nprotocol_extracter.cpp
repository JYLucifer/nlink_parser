...
void NProtocolExtracter::AddProtocol(NProtocolBase *protocol) {
  ...
  if (protocol->fixed_header().size() > static_cast<size_t>(max_header_size_)) {
...
void NProtocolExtracter::RemoveProtocol(NProtocolBase *protocol) {
  ...
  for (auto p : protocols_) {
    if (p->fixed_header().size() > static_cast<size_t>(max_size)) {
...
void NProtocolExtracter::AddNewData(const std::string &data) {
  ...
  size_t incomplete_index = data_array.size();
  ...
  for (const auto &sort_info : sort_infos) {
    ...
    if (static_cast<size_t>(header_index) < static_cast<size_t>(index_begin))
      continue;
  ...
  if (incomplete_index + max_header_size_ - 1 <= data_array.size()) {
    ...
    if (static_cast<size_t>(index_begin + max_header_size_ - 1) <= data_array.size()) {
      prev_data_array_ =
